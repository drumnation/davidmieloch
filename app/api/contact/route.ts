...
import { submitLeadIntake } from "./lead-intake";
...
export async function POST(request: Request) {
  try {
    ...
    const leadIntake = await submitLeadIntake({
      name,
      email,
      company,
      topic,
      ...
      details: {
        ...
        pressTopic,
        pressTiming,
        attachment: jobFile ? jobFile.name : undefined,
      },
    });
...
`;
    ...
    htmlText += buildHtmlEmailSection(
      "Email",
...
${section}`;
        htmlText +=
          `<hr><h2>Job Opportunity Details:</h2>` +
...
${section}`;
        htmlText +=
          `<hr><h2>Freelance / Consulting Details:</h2>` +
...
${section}`;
        htmlText +=
          `<hr><h2>Press / Speaking Details:</h2>` +
    ...
    if (!smtpConfigured) {
      if (leadIntake.success) {
        return NextResponse.json({ success: true });
      }

      console.error(
        "Missing SMTP config and lead intake did not capture the message",
      );
      return NextResponse.json(
