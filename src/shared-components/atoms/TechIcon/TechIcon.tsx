...
export const TechIcon: React.FC<TechIconProps> = ({
  ...
  size = 24,
  color,
  showLabel = false, // Keep this for inline labels
  labelPosition = 'bottom',
  className,
  onClick,
  showTooltip = true, // Use this to control hover/focus tooltips
}) => {
  const normalizedName = name.toLowerCase().trim();
  const techData = TECH_ICON_MAP[normalizedName];

  const iconContent = techData ? (
    <S.IconWrapper>
      <techData.icon size={size} color={color || techData.color} />
    </S.IconWrapper>
  ) : (
  ...
  return (
    // Wrap the main container with Mantine Tooltip
    <Tooltip
      label={name} // Tooltip content is the tech name
      withArrow
      position="top" // Or adjust as needed
      // Trigger on touch, and hover/focus based on showTooltip prop
      events={{ hover: showTooltip, focus: showTooltip, touch: true }}
      // Prevent tooltip from showing if showTooltip is false and showLabel is true (label already visible)
      disabled={!showTooltip && showLabel}
    >
      <S.TechIconContainer
        className={className}
        onClick={onClick ? () => onClick() : undefined}
        style={{ cursor: onClick ? 'pointer' : 'default' }}
        // Remove title attribute as Tooltip handles it
        // title={showTooltip ? name : undefined}
        $labelPosition={labelPosition}
      >
        {iconContent}
        {/* Conditionally render the inline label */}
