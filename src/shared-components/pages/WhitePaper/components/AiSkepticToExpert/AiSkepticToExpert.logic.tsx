...
import { HeroProps, AnimationType } from '@shared-components/organisms/Hero/Hero.types';
...
export const enhanceHeroProps = (heroProps: AiSkepticToExpertProps['heroProps'] = defaultContent.hero): HeroProps => {
  ...
  // Map the incoming animation string to a valid AnimationType or default to 'fade-up'
  let validAnimation: AnimationType = 'fade-up'; // Default

  // Check against the valid AnimationType values
  if (animation === 'fade-up' || animation === 'none' || animation === 'fade-in' || animation === 'slide-in-left' || animation === 'slide-in-right') {
    validAnimation = animation;
  } else if (animation === 'slide-in') { 
    // Handle the potentially intended 'slide-in' value - map to 'slide-in-left' as a default
    validAnimation = 'slide-in-left'; 
  }
    
  return {
    ...heroProps,
    animation: validAnimation, // Now correctly typed
...
export const useAiSkepticToExpertLogic = ({ content = defaultContent, onImageLoad }: AiSkepticToExpertProps) => {
  ...
  // Ensure enhanceHeroProps is called correctly without extra props
